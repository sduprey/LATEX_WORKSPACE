...
\end{slide}
\begin{slide}
\slideheading{In-rank computation}
...
\end{slide}
\begin{slide}
\slideheading{Exhaustive brute force doesn't work}
...
\end{slide}
\begin{slide}
\slideheading{Picturing our smallest store : jewelery }
\begin{center}
\epsfig{file=page_rank.eps,height=6.5cm,width=8.cm}
\end{center}
\end{slide}
\begin{slide}
\slideheading{Heuristic based algorithm}
We want here to optimize the adequation of our mesh $\left(X_i\right)$ to our potentiality vector $f$.
\begin{equation}
\max_{\left(G_{ij}\right)  \in \left\{0,1\right\}^{N\times N}}\left\{ \sum^{N}_{i=1} trafic\left(X_i\right)\times pageRank(X_i)\right\}
\end{equation} 
\end{slide}
\begin{slide}
\slideheading{Genetic algorithm}
\textbf{\textcolor{blue}{\large What a genetic algorithm }}
\small{
...
\newline \textbf{\textcolor{blue}{\large Why a genetic algorithm }}
\small{
